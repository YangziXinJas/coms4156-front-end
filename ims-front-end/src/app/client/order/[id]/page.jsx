...
export default function OrderDetail({params}) {
  ...
  const fetchData = async () => {
    ...
    return await Promise.all(orders.map(async (order) => {
      const itemResponse = await fetch(`http://localhost:8001/item/get/${order.itemId}`, {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
          "Authorization": "Bearer " + user.token
      ...
      if (!itemResponse.ok) {
        console.log("Fetching order information in order details page failed");
      }

      const item = await itemResponse.json();
      console.log(`Got item name: ${item.name}`);
      return {
        ...order,
        itemName: item.name,
      };
  ...
  useEffect(() => {
    fetchData().then((res) => {
      ...
      // handle misc. errors such as network errors
